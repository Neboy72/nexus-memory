...
// F9 (W39): Koerzions-Matrix der Host/Model-Eingaben ergänzt
await t("clampInt: Booleans, Partial-Nummern und whitespace-Strings", () => {
  assert.strictEqual(clampInt(true, 5, 1, 50), 5, "boolean ist kein gültiger numerischer Wert → default")
  assert.strictEqual(clampInt(false, 5, 1, 50), 5)
  assert.strictEqual(clampInt("   ", 5, 1, 50), 5, "whitespace-only → default")
  assert.strictEqual(clampInt("12abc", 5, 1, 50), 5, "partial-numerisch ist Müll → default")
  assert.strictEqual(clampInt("12", 5, 1, 50), 12, "reine Zahl bleibt valide")
})
...
await t("search.ts: limit läuft durch clampInt (verdrahtet + Schema-Minimum)", () => {
  const src = read("./tools/search.ts")
  // W39: lose Form — beliebige Whitespace/Argument-Reihenfolge-Toleranz, aber BEIDE
  // Verdrahtungspunkte (clamp-Aufruf + keine Rohweiterreichung) bleiben bewiesen.
  assert.match(src, /clampInt\(\s*params\.limit\s*,[^)]*SEARCH_LIMIT_MAX\s*\)/, "params.limit muss clampInt mit SEARCH_LIMIT_MAX durchlaufen")
  assert.match(src, /clampInt\(\s*params\.limit\s*,[^)]*SEARCH_LIMIT_DEFAULT/, "default-Parameter muss SEARCH_LIMIT_DEFAULT sein")
  assert.ok(!/const limit = params\.limit \?\? /.test(src), "roher params.limit darf nicht mehr durchgereicht werden")
})

await t("graph_traverse.ts: alle drei Limits/Depths laufen durch clampInt", () => {
  const src = read("./tools/graph_traverse.ts")
  for (const [param, def, max] of [
    ["max_depth", "TRAVERSE_DEPTH_DEFAULT", "TRAVERSE_DEPTH_MAX"],
    ["limit", "ENTITIES_LIMIT_DEFAULT", "ENTITIES_LIMIT_MAX"],
    ["max_depth", "SUBGRAPH_DEPTH_DEFAULT", "SUBGRAPH_DEPTH_MAX"],
  ]) {
    const re = new RegExp(`clampInt\\(\\s*params\\.${param}\\s*,[^)]*${max}\\s*\\)`)
    assert.match(src, re, `params.${param} muss durch clampInt mit ${max} laufen`)
  }
})
...
await t("Schemata spiegeln die Maxima (minimum/maximum)", () => {
  ...
  // W39/F8: Minima dürfen nicht stillschweigend verschwinden
  assert.match(graph, /minimum: 1/, "graph-Schemata brauchen minimum: 1")
  assert.match(search, /maximum: SEARCH_LIMIT_MAX/)  // (Duplikat-Schutz gegen Verschieben)
})
